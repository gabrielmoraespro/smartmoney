...
export default function ConectarBanco() {
  ...
  const getConnectToken = async (): Promise<string> => {
    ...
    const tokenRes = await fetchJson<{ accessToken?: string; error?: string }>(
      `${functionsBaseUrl}/pluggy-token`,
      {
        method: 'POST',
    ...
    if (!tokenRes.ok)
      throw new Error(tokenRes.data.error ?? 'Falha ao gerar token de conexão.')

    return tokenRes.data.accessToken as string
  }

  const handleManualSync = async () => {
    const itemIds = accounts
      .map((a) => a.pluggy_item_id)
    ...
    try {
      ...
      toast.success(`${total} transações atualizadas com sucesso!`, {
        id: loadingId,
      })
    } catch (error: any) {
  ...
  const openPluggy = async () => {
    ...
    try {
      ...
      if (msg.includes('404')) {
        toast.error(
          'Funções Netlify não encontradas. Use netlify dev.',
          { id: loadingId },
        )
      } else {
  ...
  const handlePluggySuccess = async ({ item }: { item: { id: string } }) => {
    ...
    try {
      ...
      await Promise.all([refreshPlanContext(), fetchConnectedAccounts()])
      toast.success(
        `Conta conectada com sucesso! ${total} transações importadas.`,
        { id: loadingId },
      )
    } catch (error: any) {
      toast.error(
        String(error.message || 'Falha ao sincronizar após conectar conta.'),
        { id: loadingId },
      )
    } finally {
      setSyncing(false)
  ...
  const handleRemoveAccount = async (account: Account) => {
    const confirmed = window.confirm(
      `Remover a conta ${account.bank_name}? Essa ação apaga as transações relacionadas.`,
    )
    if (!confirmed) return
  ...
  if (isLoading) {
    return <div className="animate-pulse p-8 text-white">Carregando...</div>
  }
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-muted-foreground">
          Conecte Nubank, Inter, Itaú e outras instituições pelo Open Finance
          (Pluggy).
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <CardHeader>
            ...
            <CardDescription>
              {isPro
                ...
                : 'Plano Free: 1 conta conectada. Upgrade libera 2 contas.'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-sm text-muted-foreground">
              Contas conectadas:{' '}
              <strong className="text-foreground">{connectedAccounts}</strong> /{' '}
              {accountLimit}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Conexão segura e criptografada via Pluggy, sem armazenar sua
              senha bancária no app.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {!canConnectAccount ? (
          ...
          <CardHeader>
            ...
            <CardDescription>
              {isPro
                ...
                : 'No plano Free você pode conectar 1 conta. Faça upgrade para conectar até 2.'}
            </CardDescription>
          </CardHeader>
          {!isPro && (
            <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Após autorizar no Pluggy, suas transações entram no dashboard
              automaticamente.
            </CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Aqui você acompanha e remove instituições conectadas.
            </CardDescription>
          </div>
          <div className="flex gap-2">
            <Button variant="default" size="sm" onClick={handleManualSync}>
              🔄 Sincronizar agora
            </Button>
            <Button
              variant="outline"
        ...
        <CardContent>
          {loadingAccounts ? (
            ...
            <p className="text-sm text-muted-foreground">
              Nenhuma conta vinculada ainda. Clique em "Conectar minha conta
              bancária".
            </p>
          ) : (

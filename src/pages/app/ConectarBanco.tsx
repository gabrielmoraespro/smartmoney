...
export default function ConectarBanco() {
  ...
  const fetchConnectedAccounts = async () => {
    ...
    if (!user) { setAccounts([]); setLoadingAccounts(false); return }
    setLoadingAccounts(true)
    const { data } = await supabase.from('accounts').select('*').eq('user_id', user.id).eq('is_active', true).order('created_at', { ascending: false })
  ...
  const handleManualSync = async () => {
    ...
    if (itemIds.length === 0) { toast.error('Nenhuma conta Pluggy para sincronizar.'); return }
  ...
  const handleRemoveAccount = async (account: Account) => {
    ...
    if (delError) { toast.error('Não foi possível remover a conta. Tente novamente.'); setRemovingId(null); return }
    if (account.pluggy_item_id) {
      const { count: remaining } = await supabase.from('accounts').select('id', { count: 'exact', head: true }).eq('pluggy_item_id', account.pluggy_item_id).eq('is_active', true)
      if ((remaining ?? 0) === 0) await supabase.from('pluggy_connections').delete().eq('pluggy_item_id', account.pluggy_item_id)
    }
          ...
          <CardContent className="space-y-4">
            {syncing && connectToken ? (
              <PluggyConnect
                ...
                onError={(error) => { toast.error(error.message); setSyncing(false); setConnectToken('') }}
                onClose={() => { setSyncing(false); setConnectToken('') }}
              />
            ) : (

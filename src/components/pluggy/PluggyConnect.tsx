...
async function loadPluggySdk(): Promise<void> {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        if (existing) {
          ...
          return reject(new Error(`SDK já carregado mas indisponível`))
        }
        const script = document.createElement('script')
        script.src = sdkUrl
        script.async = true
        script.onload = () => resolve()
        script.onerror = () => reject(new Error(`Falha ao carregar ${sdkUrl}`))
  ...
  throw new Error('Não foi possível carregar o widget Pluggy. Tente novamente.')
}
...
export function PluggyConnect({ connectToken, onSuccess, onError, onClose, disabled, autoOpen = false }: PluggyConnectProps) {
  ...
  const handleOpen = useCallback(async () => {
    if (!connectToken) {
      onError({ message: 'Connect token inválido.' })
      return
    }
    setOpening(true)
    try {

...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Conta itens únicos já conectados, não contas individuais
    const { count: itemCount } = await supabase
      .from('pluggy_connections')
      .select('id', { count: 'exact', head: true })
      .eq('user_id', user.id);

    const isNewItem = !(await supabase.from('pluggy_connections').select('id').eq('pluggy_item_id', pluggyItemId).eq('user_id', user.id).maybeSingle()).data;

    if (isNewItem && (itemCount ?? 0) >= accountLimit) {
      return {
        statusCode: 403,
        body: JSON.stringify({
          error: `Seu plano permite até ${accountLimit} instituição(ões). Remova uma ou faça upgrade.`,
        }),
      };
    }
